...
function do_featurecounts () {
	date
	########################################################################################################################
#	GTF="/data/NHLBI_BCB/Fayaz/24-lncRNA-database-curation/10-modify_lncRNAKB_GTF/lncRNAKB_hg38_v1.gtf"
#	GTF="/data/NHLBI_BCB/Fayaz/24-lncRNA-database-curation/10-modify_lncRNAKB_GTF/lncRNAKB_hg38_v5_GB.gtf"	
	GTF="/data/NHLBI_BCB/Fayaz/24-lncRNA-database-curation/10-modify_lncRNAKB_GTF/lncRNAKB_hg38_v6.gtf"	
	bamfile="/data/NHLBI_BCB/Fayaz/21-GTEx-data/03-trimmomatic-alignment-hisat2/$tissue/$SAMPLE/$SAMPLE.unique.bam" # only using uniquely mapped BAM to count, if using nonunique BAM please check -M option for featureCounts
	...
	feature="gene" # if not gene, please leave "blank"
		...
		then
			...
			then
				...
				featureCounts -T $numcpus \
					...
					-a $GTF \
					-s $s \
					-o $outdir/$tissue/$SAMPLE/$out_dir/$SAMPLE.genefeatureCounts.txt \
					$bamfile
			else
			...
			then
				...
				featureCounts -T $numcpus \
					...
					-a $GTF \
					-s $s \
					-o $outdir/$tissue/$SAMPLE/$out_dir/$SAMPLE.exonfeatureCounts.txt \
					$bamfile
...
function do_salmon_index () {
	...
	$salmon index --type quasi \
		      -p $numcpus \
                      ...
                      --gencode \
		      -t $transcript_ref \
		      -i /home/seifuddinft/salmon_transcript_index_gencodev29hg38
...
#	READ2="${SAMPLE}_2P.fastq.gz"
	...
	$salmon quant -i /home/seifuddinft/salmon_transcript_index_gencodev29hg38 \
		      -l A \
		      -1 "$DATAPATH/$READ1" \
		      -2 "$DATAPATH/$READ2" \
		      -p $numcpus \
		      -g $GTF \
		      --validateMappings \
	              -o $outdir/$SAMPLE/$out_dir/${SAMPLE}.salmon
...
function do_featurecounts_merge () {
	date
	########################################################################################################################
#       sids="/data/NHLBI_BCB/Fayaz/21-GTEx-data/00-by-tissue-files-and-ids/Blood_ID_R1_R2.txt"
	feature="gene"
	out_dir="featurecounts"
	
	for i in `cat $sids | cut -d" " -f1 | head -1`; do
           SAMPLE=$i
	   echo -e "$feature\t$i" > $outdir/$tissue/tmp1
	   cat $outdir/$tissue/$SAMPLE/$out_dir/$i.${feature}featureCounts.txt | sed '1,2d' | sort -k1,1 | cut -f 1,7 >> $outdir/$tissue/tmp1
	done

	for i in `cat $sids | cut -d" " -f1 | sed '1,1d' `; do
           SAMPLE=$i
	   echo -e "$i" > $outdir/$tissue/tmp2
	   cat $outdir/$tissue/$SAMPLE/$out_dir/$i.${feature}featureCounts.txt | sed '1,2d' | sort -k1,1 | cut -f 7 >> $outdir/$tissue/tmp2
	   paste -d"\t" $outdir/$tissue/tmp1 $outdir/$tissue/tmp2 > $outdir/$tissue/tmp3
	   mv -f $outdir/$tissue/tmp3 $outdir/$tissue/tmp1
	done

	mv -f $outdir/$tissue/tmp1  $outdir/$tissue/$tissue.$feature.featureCounts.txt
	rm -f $outdir/$tissue/tmp2
	rm -f $outdir/$tissue/tmp3
	########################################################################################################################
	date
}
...
#	count_out_dir="$outdir/04-salmon-transcript-gene-counts-tpm"
#	mkdir $count_out_dir/$feature
#	mkdir $count_out_dir/$feature/$typeofcount
	count_out_dir="$outdir/04-salmon-transcript-gene-counts-tpm/$feature/$typeofcount"

	for i in `cat $sids | cut -f1 | head -1`; do
           SAMPLE=$i
	   echo -e "$feature\t$i" > $count_out_dir/tmp1
	   cat $outdir/$SAMPLE/$out_dir/$SAMPLE.salmon/quant.genes.sf | sed '1,2d' | sort -k1,1 | cut -f 1,5 >> $count_out_dir/tmp1
	done

	for i in `cat $sids | cut -f1 | sed '1,1d'`; do
           SAMPLE=$i
	   echo -e "$i" > $count_out_dir/tmp2
	   cat $outdir/$SAMPLE/$out_dir/$SAMPLE.salmon/quant.genes.sf | sed '1,2d' | sort -k1,1 | cut -f 5 >> $count_out_dir/tmp2
	   paste $count_out_dir/tmp1 $count_out_dir/tmp2 > $count_out_dir/tmp3
	   mv -f $count_out_dir/tmp3 $count_out_dir/tmp1
	done

	mv -f $count_out_dir/tmp1  $count_out_dir/$feature.salmon.$typeofcount.txt
	rm -f $count_out_dir/tmp2
	rm -f $count_out_dir/tmp3
	########################################################################################################################
	date
}
...
#	logfiles="/mnt/lab-thein/Fayaz/00-fastqc-trimmomatic-hisat2-stringtie-featurecounts/01-set1-log-files-hisat2"	
	...
	rm -f tmp*
	a1=`cat alignment.stats | sed 's/,//g' | sed '1,1d'  | awk '{ sum += $2; n++ } END { print int(sum / n); }' | awk '{ printf("%'"'"'d\n",$1); }'`
	a2=`cat alignment.stats | sed 's/,//g' | sed '1,1d'  | awk '{ sum += $3; n++ } END { print sum / n; }' | awk -F. '{print $1"."substr($2,1,2)"%"}'` 
	a3=`cat alignment.stats | sed 's/,//g' | sed '1,1d'  | awk '{ sum += $4; n++ } END { print int(sum / n); }' | awk '{ printf("%'"'"'d\n",$1); }'`
	a4=`cat alignment.stats | sed 's/,//g' | sed '1,1d'  | awk '{ sum += $5; n++ } END { print sum / n; }' | awk -F. '{print $1"."substr($2,1,2)"%"}' `
...
function do_get_featurecount_stats () {
	date
	########################################################################################################################
# 	sids="/data/NHLBI_BCB/Fayaz/21-GTEx-data/00-by-tissue-files-and-ids/Heart_ID_R1_R2.txt"
	feature="gene"
	
	# change log files directory to absolute path of logfiles
	logfiles="$outdir/$tissue/"

	cd $logfiles
	
#	echo -e "Tissue\tSample\t${feature}_Count\tTotal_fragments\tSuccessfully_assigned\t%" > $tissue.$feature.featurecount.summary.txt

#	for i in `cat $sids | cut -d" " -f1`; do
#	   count_of_features=`cat $i.$tissue.slurm.err.txt | grep 'Meta-features : ' | awk '{print $4}'`
#	   total=`cat $i.$tissue.slurm.err.txt | grep 'Total alignments : ' | awk '{print $5}'`
#	   s=`cat $i.$tissue.slurm.err.txt | grep 'Successfully assigned alignments : ' | sed 's/(//g' | sed 's/)//g' |awk '{print $6"\t"$7}'`
#	   echo -e "$tissue\t$i\t$count_of_features\t$total\t$s" >> $tissue.$feature.featurecount.summary.txt
#	done

	cat $tissue.$feature.featurecount.summary.txt | awk '{if($5<10000000){print $0}}' | cut -f2 > ${tissue}_subjects_to_exclude.txt
	########################################################################################################################
	date
}
