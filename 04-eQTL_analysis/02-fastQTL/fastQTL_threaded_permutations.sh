...
singularity exec /data/NHLBI_BCB/Fayaz/21-GTEx-data/09-fastQTL_threaded/gtex_eqtl_V8.sif /opt/fastqtl/python/run_FastQTL_threaded.py --covariates  $inputdir/$tissue/$covariatedatafilename.covariate.sorted.txt \
					--window $window \
					--chunks $chunks \
					--threads $threads \
					--permute $permutations1 $permutations2 \
					-o $outputdir/$tissue/ \
$vcfinputdir/$tissue/$vcffilename.subset.GTEx.subset.MAF0.05.FINAL.sorted.vcf.gz $inputdir/$tissue/$expressiondatafilename.sorted.bed.gz $covariatedatafilename.subset.GTEx.subset.MAF0.05.FINAL.win.$window.bp.perm1.$permutations1.perm2.$permutations2.cis.nominal.threaded.eqtl
