...
# F4310 = 4_310/310.chr_bp_gene_dedup   +   4_310/gene_subtract.chr_bp_gene_dedup  ((46421+7157)53578  +  10506 )  + 20700
for i in `ls $wdir_prev3/gene_subtract/chr*/chr* `; do 
   cat $i | head -1 | cut -f 1,4,5,7,9 | cut -f 1 -d';' | sed 's/gene_id "//g' | sed 's/"//g' >> $wdir_prev3/gene_subtract.chr_bp_gene_dedup
done
cat $wdir_prev3/gene_subtract.chr_bp_gene_dedup | wc -l  #20700 
cat  $wdir_prev3/310.chr_bp_gene_dedup   $wdir_prev3/gene_subtract.chr_bp_gene_dedup   > $wdir/4310.chr_bp_gene_dedup  #84784
F4310="$wdir/4310.chr_bp_gene_dedup"
...
if [ -e $wdir_prev3/gene_subtract/$chr2/${chr2}_${st2}_${en2} ]
then
...
# 5) for each transcript in this gene (../05-MITRANSCRIPTOMEv2/mitranscriptome_trans/$chr/${chr}_${st}_${en}/*) check if it is in boundry 
...
for i in `ls ../05-MITRANSCRIPTOMEv2/mitranscriptome_trans/$chr/${chr}_${st}_${en}/*`; do
...
cat $wdir/4310.chr_bp_gene_dedup | cut -f 5 | cut -f 2 -d'.' | sort -n  | tail -1 > $wdir/last_gene_id_num  #98333 last number

# 2) for each transcript add .1 .2 .3 etc 
# per gene
m=`cat  $wdir/$wdir.subtract | wc -l`  # 15164
...
c=1 ; d=0 ; n=0 ; cat  $wdir/$wdir.subtract  |   while read chr st en strand g1 ; do
  ...
  cat ../05-MITRANSCRIPTOMEv2/mitranscriptome_genes/$chr/${chr}_${st}_${en} | head -1 | sed 's/GENE_NAME=/gene_alias "/g'  \
